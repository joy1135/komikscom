...
async def seed():
    ...
    async with async_session() as session:
        ...
        # Пользователи
        admin_user = m.User(email="admin@example.com", nick="admin", password="admin123", roleId=admin_role.id)
        regular_user = m.User(email="user@example.com", nick="user", password="user123", roleId=user_role.id)
        aftor_user = m.User(email="aftor@example.com", nick="aftor", password="aftor123", roleId=aftor_role.id)
        ...
        # Для каждого комикса добавляем том 1 / главу 1 и копируем страницы
        for comic in [comic1, comic2]:
            ...
            # Путь назначения
            target_chapter_folder = os.path.join(
                COMICS_ROOT, comic.title, "comic", "vl1", "ch1"
            )
